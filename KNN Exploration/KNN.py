...
# CSV file path
file_path = f"./{prefix}_Data_normalised_Train_Set.csv"
...
def hyperparameter_search(X, y, param_grid):
    ...
    # Add accuracy score to the results
    results["mean_accuracy"] = cross_val_score(grid.best_estimator_, X, y, cv=cv, scoring="accuracy").mean()
    ...
    top10.to_csv(f"{prefix}_knn_gridsearch_top10.csv", index=False)
